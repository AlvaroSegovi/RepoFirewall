...
#Se asoció a esta subnet la tabla de rutas ya que aqui estará el servidor que saldrá a internet por el firewall 
...
#Crear la regla DNAT
az network firewall nat-rule create --firewall-name AzureFirewall --resource-group FWRG --collection-name Regla1RDP --priority 101 --action DNAT --name vmaccess --source-addresses 170.84.135.64 --protocols TCP --destination-ports 3500 --destination-addresses 20.232.174.4 --translated-address 10.0.2.4 --translated-port 3389
